...
:mod:`kintsugi.taxonomy.codes`. The held-out strings were never looked at while
...
RULES: tuple[Rule, ...] = (
    ...
    _rule(r"cancelled by user|user[_ ]aborted|declined the collect request",
    ...
    _rule(r"^54\b|expired card|invalid card number|vpa does not exist"
          r"|invalid[_ ]instrument[_ ]details",
          FailureClass.INVALID_INSTRUMENT),
    _rule(r"^14\b|account closed|dormant|account[_ ]inactive"
          r"|beneficiary account frozen",
          FailureClass.ACCOUNT_CLOSED),
    _rule(r"mandate has been revoked|umn not found|umn.*cancelled"
          r"|subscription[_ ]cancelled",
          FailureClass.MANDATE_REVOKED),

    # --- balance ---------------------------------------------------------
    _rule(r"^51\b|\bZ9\b|insufficient (funds|balance)|\bNSF\b"
          r"|does not have sufficient balance|low balance",
          FailureClass.INSUFFICIENT_FUNDS),

    # --- limits ----------------------------------------------------------
    _rule(r"^61\b|^65\b|\bU67\b|exceeds withdrawal|per transaction limit"
          r"|daily limit|txn[_ ]limit[_ ]breached|limit.*reached",
